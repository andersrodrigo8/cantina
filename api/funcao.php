<?php
    ...
    //--------------------------------SOMA TOTAL DE VALORES PRODUTOS EM ESTOQUE----------------------------------//
    function somaTotaldosValoresEstoque($db){
        $sql = "select * from public.tbproduto where tbproduto.quantidade > 0";
        $queryValorTotal = selectpadrao($db,$sql);
        $soma = 0;
        foreach($queryValorTotal as $resultado){
            $soma = $soma + ($resultado['quantidade'] * $resultado['valor']);
    ...
    //--------------------------------SELECIONA TODOS USUARIOS ADMINSTRADORES----------------------------------//
    function selectUsuariosAdministradores($db){
        $listaemail = null;
        $sql = "select * from public.tbusuario where tipo = 'ADMINISTRADOR'";
        $query = selectpadrao($db,$sql);
        for($i=0; $i<count($query); $i++){
            if($i == 0){
                $listaemail .= $query[$i]['email'];
            }else{
                $listaemail .= ';'.$query[$i]['email'];
            }
        }
    ...
    function selectUsuarioConsumoGeral($db){
        //$sql = "select tbusuario.nome, sum(valortotal) as total from public.tbusuario, public.tbvenda, public.tbvendaitens where tbvendaitens.vend_id = tbvenda.id and tbvenda.status = 'PENDENTE' and tbusuario.id = tbvendaitens.usua_id group by tbusuario.nome";
	$sql = "select tbusuario.nome, sum(valortotal) as total from public.tbusuario, public.tbvenda, public.tbvendaitens where tbvendaitens.vend_id = tbvenda.id and tbvenda.status = 'PENDENTE' and tbusuario.id = tbvendaitens.usua_id group by tbusuario.nome order by total desc"; 
	$query = selectpadrao($db,$sql);
        return $query;
    }
    ...
    //--------------------------------LISTA TODOS OS PRODUTOS QUANTIDADE MAIOR QUE ZERO----------------------------------//
    ...
    function selectProdutos($db){
        $sql = "select * from public.tbproduto order by tbproduto.titulo asc;";
        $query = selectpadrao($db,$sql);
        return $query;
    }
    ...
    function selectProdutoValidade($db){
        ...
        foreach($query as $resultado){
            $produtos .= '<br>' . $resultado['titulo'] . ' - '.date('d/m/Y', strtotime($resultado['validade']));
        }
        return $produtos;
    }

    //--------------------------------UPLOAD DE ARQUIVOS NOTAS----------------------------------//
    function enviandoArquivoNota($nota){
        ...
        $extensao = strtolower(end(explode('.', $nota)));
    ...
    if(isset($_POST['botaoAcao']) && $_POST['botaoAcao'] == "CadastroUsuario"){
        ...
        if(!empty($re) && !empty($nome) && !empty($email) && !empty($senha) && !empty($tipo)){
            ...
            $status = inserirRegistro($db, $sqlInsert);
    ...
    if(isset($_POST['id']) && !empty($_POST['id']) && isset($_POST['botaoAcao']) && $_POST['botaoAcao'] == "AlterarUsuario"){
        ...
        if(!empty($id) && !empty($re) && !empty($nome) && !empty($email) && !empty($senha) && !empty($tipo)){
            ...
            $status = alterarRegistro($db, $sqlUpdate);             
    ...
    if(isset($_POST['botaoAcao']) && $_POST['botaoAcao'] == "CadastroProduto"){
		...
		$sqlInsert = "insert into public.tbproduto(titulo, descricao, quantidade, valor, validade, promocao, pathfoto) SELECT '".pg_escape_string($titulo)."', '".pg_escape_string($descricao)."', ".pg_escape_string($quantidade).", '".pg_escape_string($valor)."', '".pg_escape_string($validade)."', '".pg_escape_string($promocao)."', '".pg_escape_string($pathfoto)."' WHERE NOT EXISTS (SELECT id FROM public.tbproduto WHERE titulo = '".pg_escape_string($titulo)."' AND descricao = '".pg_escape_string($descricao)."' AND quantidade = ".pg_escape_string($quantidade)." AND valor = '".pg_escape_string($valor)."');";
            }else{
		...
		$sqlInsert = "insert into public.tbproduto(titulo, descricao, quantidade, valor, validade, promocao) SELECT '".pg_escape_string($titulo)."', '".pg_escape_string($descricao)."', ".pg_escape_string($quantidade).", '".pg_escape_string($valor)."', '".pg_escape_string($validade)."', '".pg_escape_string($promocao)."' WHERE NOT EXISTS (SELECT id FROM public.tbproduto WHERE titulo = '".pg_escape_string($titulo)."' AND descricao = '".pg_escape_string($descricao)."' AND quantidade = ".pg_escape_string($quantidade)." AND valor = '".pg_escape_string($valor)."');";
            }
            $status = inserirRegistro($db, $sqlInsert);
    ...
    if(isset($_POST['id']) && !empty($_POST['id']) && isset($_POST['botaoAcao']) && $_POST['botaoAcao'] == "AlterarProduto"){
        ...
        if(!empty($id) && !empty($titulo) && !empty($descricao) && !empty($valor) && !empty($validade)){
            if(!empty($pathfoto) && $pathfoto != "") {
    ...
    if(isset($_POST['botaoAcao']) && $_POST['botaoAcao'] == "InserirNota"){
        $nota = $_FILES['arquivo']['name'];


        if(!empty($nota) && $nota != ""){
            $nota = enviandoArquivoNota($nota);
        }

    }

    if(isset($_POST['botaoAcao']) && $_POST['botaoAcao'] == "ExcluirNota"){
        $nota = $_POST['arquivopath'];
